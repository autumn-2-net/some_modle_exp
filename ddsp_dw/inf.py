...
from ddsp_dw.logger import utils
from diffwave.params import AttrDict, params as base_params
from diffwave.params import  params as sss
#from diffwave.model import DiffWave
from PL_diffwav_modle_ddsp import PL_diffwav

models = {}

def predict(f0,spectrogram=None, model_dir=None, params=None, device=torch.device('cuda'), fast_sampling=False):
  # Lazy load model.
  if not model_dir in models:
   ...
   # model = PL_diffwav(params=base_params).load_from_checkpoint(r"./default/version_57/checkpoints/epoch=23-step=253292.ckpt",params=base_params)
    ...
    model = PL_diffwav(params=base_params, argss=args).load_from_checkpoint(
      r"./bignet_mix_mel/default/version_1/checkpoints/epoch=4-step=10611.ckpt", params=base_params, argss=args)
  ...
  with torch.no_grad():
    ...
    for n in tqdm(range(len(alpha) - 1, -1, -1)):
      ...
      audio, wavv, s_h = model(audio, torch.tensor([T[n]], device=audio.device), f0,
                                      spectrogram)
  ...
  return audio, params.sample_rate, wavv


def main(args):
  if args.spectrogram_path:
    spectrogram = torch.from_numpy(np.load(args.spectrogram_path))
    f0=torch.from_numpy(np.load(args.spectrogram_path.replace('spec','f0'))).unsqueeze(0)
  else:
    spectrogram = None

  audio, sr,ddsp = predict(f0,spectrogram, model_dir=args.model_dir, fast_sampling=args.fast, params=base_params)
...
if __name__ == '__main__':
 ...
 #  dddd=torch.load('./o2/左手指月.mel.pt')
  ...
  parser.add_argument('--spectrogram_path', '-s', default='./test/2099003695.wav.spec.npy',
                      help='path to a spectrogram file generated by diffwave.preprocess')
  # parser.add_argument('--spectrogram_path', '-s', default='./o2/一半一半.mel.pt',
  #                     help='path to a spectrogram file generated by diffwave.preprocess')
  # parser.add_argument('--spectrogram_path', '-s', default='./t/50.wav.spec.npy',
  #                     help='path to a spectrogram file generated by diffwave.preprocess')

  parser.add_argument('--output', '-o', default='./o2/fsnnnzybybzss.wav',
