...
class V3WLR(_LRScheduler):
    ...
    def ctxadjust_lrs(self, T_0=30000, eta_min=5e-6, eta_max=2e-4, tmctx=0.85, ws=5000):
        step_num = self.last_epoch + 1  # +75120 +169020#+360000

        if ws > step_num:
            cur_lr = step_num * (eta_max / ws)
        else:
            cur_lr = eta_max * (tmctx ** ((step_num - ws) // T_0))
        if cur_lr < eta_min:
            cur_lr = eta_min
        return cur_lr
